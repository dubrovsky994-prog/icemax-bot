// 🤖 ICEMan Bot для MAX | Заправка автокондиционеров
// 🔗 Деплой: Vercel (бесплатно)

const BOT_TOKEN = "f9LHodD0cOKZBlFn6bQGSi_mJfHTHJTrmy4dgzwZCScmBTcLbmtN3ubTTKPHZN3CuZ4zvi_gsOwfwv3AiQ9L";
const ADMIN_CHAT_ID = "229831120";

export default async function handler(req, res) {
    // Разрешаем CORS и только POST-запросы
    ...
    try {
        ...
        const chat_id = msg.chat_id || msg.chat?.id;
        const text = (msg.text || "").toLowerCase();
        const user_name = msg.from?.name || "Клиент";

        if (!chat_id) return res.status(200).send("OK");

        // 🔹 ОТВЕТЫ БОТА (меняй текст здесь)
        let reply = "";
        ...
        // Отправляем ответ если нашли
        if (reply) {
            await sendMaxMessage(chat_id, reply);
            return res.status(200).send("OK");
        }

        // Пересылаем админу если не нашли ответ
